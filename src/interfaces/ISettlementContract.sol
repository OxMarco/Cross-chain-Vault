...
interface ISettlementContract {
    ...
    function initiate(CrossChainOrder memory order, bytes memory signature) external;

    /// @notice Resolves a specific CrossChainOrder into a generic ResolvedCrossChainOrder
    /// @dev Intended to improve standardized integration of various order types and settlement contracts
    /// @param order The CrossChainOrder definition
    /// @return ResolvedCrossChainOrder hydrated order data including the inputs and outputs of the order
    function resolve(CrossChainOrder memory order) external view returns (ResolvedCrossChainOrder memory);
}
